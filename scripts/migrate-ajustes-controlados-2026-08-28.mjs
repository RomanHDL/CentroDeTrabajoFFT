...
async function renameInPlace(
  ...
  oldName,
  newName,
  { role, requiredRoleLabel, category } = {},
) {
...
console.log(
  '--- 1) Renombrar "Limpieza"/"Limpieza 2" -> "Limpieza de TV"/"Limpieza de TV 2" en WC LINEA 0-10 ---',
)
...
for (const areaCode of [
  ...
  await renameInPlace(areaCode, 'Limpieza', 'Limpieza de TV', {
    role: 'Limpieza de TV',
    requiredRoleLabel: 'Auxiliar de Limpieza',
  })
  await renameInPlace(areaCode, 'Limpieza 2', 'Limpieza de TV 2', {
    role: 'Limpieza de TV',
    requiredRoleLabel: 'Auxiliar de Limpieza',
  })
}
...
await renameInPlace(
  ...
  {
    role: 'Ayudante General — Protectores Espuma',
    requiredRoleLabel: 'Ayudante General — Protectores Espuma',
  },
)

console.log(
  '--- 3) Renombrar "Ayudante General Conveyor N" -> "Ayudante General de Conveyor N" en WC Paletizado ---',
)
for (const n of [1, 2]) {
  await renameInPlace(
    'PALETIZADO',
...
console.log(
  '--- 5) Desactivar rol generico "Conveyor" (Conveyor 1/2) en WC Paletizado -- reemplazado por "Ayudante General de Conveyor" ---',
)
await deactivate('PALETIZADO', 'Conveyor 1')
await deactivate('PALETIZADO', 'Conveyor 2')

console.log(
  '--- 6) Desactivar puestos propios de WC Conveyor General (fusionado en Paletizado, ya no tiene puestos aparte) ---',
)
