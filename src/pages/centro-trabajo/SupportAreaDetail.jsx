...
function describeAreaState(real, ideal) {
  if (ideal == null) return { tone: 'ok', Icon: CheckCircleIcon, label: 'Sin plantilla oficial', description: 'Esta área no tiene una plantilla ideal definida todavía.' }
  if (real === 0) return { tone: 'bad', Icon: WarningAmberIcon, label: 'Sin personal', description: 'Actualmente no hay personal asignado a esta área.' }
  if (real > ideal) {
    const extra = real - ideal
    return { tone: 'ok', Icon: CheckCircleIcon, label: 'Completa', description: `Cuenta con ${extra} persona${extra === 1 ? '' : 's'} adicional${extra === 1 ? '' : 'es'} a la plantilla ideal.` }
  }
  if (real === ideal) return { tone: 'ok', Icon: CheckCircleIcon, label: 'Completa', description: 'Cuenta con el personal ideal asignado.' }
  const missing = ideal - real
  return { tone: 'warn', Icon: WarningAmberIcon, label: real / ideal >= 0.5 ? 'Parcial' : 'Falta personal', description: `Requiere ${missing} persona${missing === 1 ? '' : 's'} adicional${missing === 1 ? '' : 'es'} para alcanzar la plantilla ideal.` }
}
      ...
      <Stack direction="row" spacing={1.5} alignItems="center">
        ...
        {assignedDate && (
          <Stack direction="row" spacing={0.5} alignItems="center" sx={{ flexShrink: 0, display: { xs: 'none', sm: 'flex' } }}>
            ...
            <Box>
              <Typography sx={{ fontSize: 9.5, color: 'text.secondary', lineHeight: 1.1 }}>Fecha asignación</Typography>
              <Typography sx={{ fontSize: 11.5, fontWeight: 700, lineHeight: 1.1 }}>{assignedDate}</Typography>
            </Box>
          </Stack>
        )}
        <Chip
  ...
  return (
    <DraggablePersonChip employeeId={person.id} sx={{ display: 'block' }}>
      <Stack
        ...
        sx={{
          p: 0.85, borderRadius: 2, border: '1px solid', borderColor: 'divider', cursor: 'pointer',
          '&:hover': { borderColor: '#3B82F6', bgcolor: (t) => alpha('#3B82F6', t.palette.mode === 'dark' ? 0.1 : 0.05) },
        }}
      >
        <EmployeeAvatar employee={person} size={30} />
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography noWrap sx={{ fontWeight: 700, fontSize: 12 }}>{person.name}</Typography>
          <Typography noWrap sx={{ fontSize: 10, color: 'text.secondary' }}>#{person.employeeNumber}</Typography>
        </Box>
...
export default function SupportAreaDetail({ workCenterId, open, onClose, previous, next, onNavigate }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/personnel/area-history?areaId=${encodeURIComponent(workCenterId)}&limit=10`, { credentials: 'include' })
  ...
  const donutData = staffing.ideal != null
    ? [
      ...
      <Box key={workCenterId} sx={{ p: { xs: 1.5, md: 3 }, overflowY: 'auto' }}>
        ...
        <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid', borderColor: 'divider', mb: 2.5, overflow: 'hidden' }}>
          <Stack direction={{ xs: 'column', md: 'row' }}>
            <Box sx={{ px: { xs: 1.5, md: 2.25 }, py: 1.25, flex: '1 1 170px', display: 'flex', alignItems: 'center', gap: 1.25 }}>
              ...
              <Box>
                ...
                <Stack direction="row" spacing={0.5} alignItems="center" sx={{ mt: 0.25 }}>
                  <Box sx={{ width: 7, height: 7, borderRadius: '50%', bgcolor: statusMeta?.color || '#94A3B8' }} />
                  <Typography sx={{ fontSize: 11, fontWeight: 700, color: statusMeta?.color || 'text.secondary' }}>{headerLabel}</Typography>
                </Stack>
              </Box>
            </Box>

            <Box sx={{ px: { xs: 1.5, md: 2.25 }, py: 1.25, flex: '1 1 190px', borderLeft: { md: '1px solid' }, borderColor: 'divider' }}>
              <Typography sx={{ fontSize: 10.5, fontWeight: 800, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.5, mb: 0.5 }}>Estado del área</Typography>
              ...
              <Typography sx={{ fontSize: 10.5, color: 'text.secondary', mt: 0.25 }}>{state.description}</Typography>
            </Box>

            <Box sx={{ px: { xs: 1.5, md: 2.25 }, py: 1.25, flex: '1 1 150px', borderLeft: { md: '1px solid' }, borderColor: 'divider' }}>
              <Typography sx={{ fontSize: 10.5, fontWeight: 800, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.5, mb: 0.5 }}>Cobertura</Typography>
              ...
              {coveragePct != null && (
                <>
                  ...
                  <Typography sx={{ fontSize: 10.5, color: 'text.secondary' }}>{staffing.real} de {staffing.ideal} personas</Typography>
                </>
              )}
            </Box>

            <Box sx={{ px: { xs: 1.5, md: 2.25 }, py: 1.25, flex: '1 1 110px', borderLeft: { md: '1px solid' }, borderColor: 'divider' }}>
              <Typography sx={{ fontSize: 10.5, fontWeight: 800, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.5, mb: 0.5 }}>Plantilla ideal</Typography>
            ...
            {description && (
              <Box sx={{
                px: { xs: 1.5, md: 2.25 }, py: 1.25, flex: '1.3 1 220px', borderLeft: { md: '1px solid' }, borderColor: 'divider',
                ...
                <Stack direction="row" spacing={0.6} alignItems="center" sx={{ mb: 0.25 }}>
                  ...
                  <Typography sx={{ fontSize: 11.5, fontWeight: 800, color: '#3B82F6' }}>Información del área</Typography>
                </Stack>
                <Typography sx={{ fontSize: 12, fontWeight: 600 }}>{description}</Typography>
                <Typography sx={{ fontSize: 10.5, color: 'text.secondary' }}>
                  {missing === 0 ? 'No requiere cobertura adicional' : `Requiere ${missing} persona${missing === 1 ? '' : 's'} para cobertura completa`}
                </Typography>
              </Box>
            )}
          </Stack>
        ...
        <Grid container spacing={2} sx={{ mb: 2.5 }}>
          <Grid item xs={12} lg={8}>
            <Paper
              ...
              sx={{
                ...
                p: 2, height: '100%', transition: 'all .15s ease',
              }}
            >
              ...
              {people.length === 0 ? (
                <EmptyState compact title="No hay personal asignado actualmente." description="Registra personal o arrastra a alguien desde 'Disponibles para asignar'." />
              ) : (
                <Stack spacing={1.25}>
                  {people.map((p) => <PersonCard key={p.id} person={p} areaId={workCenterId} canManage={isSupervisor} />)}
                </Stack>
              )}
            </Paper>
          </Grid>
          <Grid item xs={12} lg={4}>
            <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid', borderColor: 'divider', p: 2, height: '100%' }}>
              <Typography sx={{ fontWeight: 800, fontSize: 14.5, mb: 1.5 }}>Resumen del área</Typography>
              <Box sx={{ position: 'relative', height: 150 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={donutData} dataKey="value" nameKey="label" innerRadius="62%" outerRadius="92%" paddingAngle={1.5} stroke="none">
                      {donutData.map((row) => <Cell key={row.key} fill={row.color} />)}
                    </Pie>
                    <RTooltip formatter={(v, n) => [`${v} persona${v === 1 ? '' : 's'}`, n]} />
                  </PieChart>
                </ResponsiveContainer>
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', textAlign: 'center', pointerEvents: 'none' }}>
                  <Typography sx={{ fontSize: 22, fontWeight: 800, lineHeight: 1 }}>{staffing.real}</Typography>
              ...
              <Stack direction="row" spacing={1} alignItems="flex-start" sx={{
                p: 1.25, borderRadius: 2, bgcolor: alpha(TONE_COLOR[state.tone], 0.1), border: `1px solid ${alpha(TONE_COLOR[state.tone], 0.25)}`,
              }}>
        ...
        <Grid container spacing={2} sx={{ mb: 2.5 }}>
          <Grid item xs={12} md={6}>
            <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid', borderColor: 'divider', p: 2, height: '100%' }}>
              <Typography sx={{ fontWeight: 800, fontSize: 13.5, mb: 1.5 }}>Disponibles para asignar ({available.length})</Typography>
              {available.length === 0 ? (
                <EmptyState compact title="No hay candidatos disponibles" description="Actualmente no hay personal disponible para asignar a esta área." />
              ) : (
                <Stack spacing={0.85}>
                  {available.slice(0, 6).map((p) => <AvailableCandidateRow key={p.id} person={p} areaId={workCenterId} />)}
                </Stack>
              )}
              {available.length > 0 && isSupervisor && (
                <Button size="small" onClick={() => setRegisterOpen(true)} sx={{ mt: 1, textTransform: 'none', fontWeight: 700 }}>
            ...
            <Paper elevation={0} sx={{ borderRadius: '16px', border: '1px solid', borderColor: 'divider', p: 2, height: '100%' }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1.5 }}>
                ...
                {history.items.length > 3 && (
                  <Button size="small" onClick={() => setShowAllHistory((v) => !v)} sx={{ textTransform: 'none', fontWeight: 700 }}>
              ...
              {history.loading ? (
                ...
                <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>No se pudo cargar la actividad reciente.</Typography>
              ) : historyItems.length === 0 ? (
                <EmptyState compact title="Sin actividad reciente" description="Todavía no hay asignaciones o movimientos registrados para esta área." />
              ) : (
                <Stack spacing={1.25}>
                  {historyItems.map((h) => (
        ...
        <Paper elevation={0} sx={{
          ...
          <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>
            {area.name} es un área de soporte. La asignación de personal se gestiona directamente por los responsables autorizados.
          </Typography>
        </Paper>
      </Box>
