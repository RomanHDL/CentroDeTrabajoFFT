...
export default function RankLegend() {
  ...
  return (
    <Stack direction="row" spacing={1.25} useFlexGap flexWrap="wrap" alignItems="center" sx={{ px: 2, pb: 1.5 }}>
      ...
      {PERSONNEL_RANK_ORDER.map((rank) => (
        <Stack key={rank.key} direction="row" spacing={0.5} alignItems="center">
          <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: rank.color, flexShrink: 0 }} />
          <Typography sx={{ fontSize: 10.5, fontWeight: 600, color: d ? 'text.secondary' : 'text.secondary' }}>
            {rank.label}
          </Typography>
        </Stack>
      ))}
