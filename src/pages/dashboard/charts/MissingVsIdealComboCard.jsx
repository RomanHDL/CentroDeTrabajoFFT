    ...
    >
      <Box sx={{ flex: 1, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 8, right: 12, left: -12, bottom: 4 }} barCategoryGap="20%">
            <CartesianGrid vertical={false} stroke={gridColor} />
            <XAxis dataKey="shortName" tick={{ fontSize: 10.5, fill: axisColor }} axisLine={{ stroke: gridColor }} tickLine={false} interval={0} angle={-20} textAnchor="end" height={48} />
            <YAxis yAxisId="left" tick={{ fontSize: 11, fill: axisColor }} axisLine={false} tickLine={false} width={32} allowDecimals={false} />
            <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11, fill: '#EF4444' }} axisLine={false} tickLine={false} width={28} allowDecimals={false} />
            <Tooltip content={<ChartTooltip />} cursor={{ fill: d ? 'rgba(255,255,255,.04)' : 'rgba(0,0,0,.03)' }} />
            <Legend wrapperStyle={{ fontSize: 11 }} formatter={(v) => ({ actual: 'Personal actual', ideal: 'Plantilla ideal', missing: 'Faltante' }[v] || v)} />
            <Bar yAxisId="left" dataKey="actual" name="actual" fill="#3B82F6" radius={[3, 3, 0, 0]} maxBarSize={22} />
            <Bar yAxisId="left" dataKey="ideal" name="ideal" fill={idealColor} radius={[3, 3, 0, 0]} maxBarSize={22} />
            <Line yAxisId="right" type="monotone" dataKey="missing" name="missing" stroke="#EF4444" strokeWidth={2} dot={{ r: 3, fill: '#EF4444', strokeWidth: 0 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </Box>
    </ChartCard>
