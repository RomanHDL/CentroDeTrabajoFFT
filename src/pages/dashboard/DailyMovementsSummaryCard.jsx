...
function Row({ icon, label, value, color }) {
  return (
    <Stack direction="row" alignItems="center" spacing={1.25} sx={{ py: 0.75 }}>
      <Box sx={{ width: 30, height: 30, borderRadius: '50%', bgcolor: `${color}22`, color, display: 'grid', placeItems: 'center', flexShrink: 0 }}>
        {icon}
      </Box>
  ...
  return (
    ...
    >
      <Stack spacing={0} divider={<Box sx={{ borderBottom: '1px solid', borderColor: 'divider' }} />}>
        <Row icon={<ArrowUpwardIcon sx={{ fontSize: 16 }} />} label="Asignaciones" value={d.asignaciones} color="#10B981" />
        <Row icon={<ArrowDownwardIcon sx={{ fontSize: 16 }} />} label="Removidos / Liberados" value={d.removidos} color="#EF4444" />
        <Row icon={<SwapHorizIcon sx={{ fontSize: 16 }} />} label="Movimientos / Reasignaciones" value={d.movimientos} color="#3B82F6" />
      </Stack>
      <Stack direction="row" alignItems="center" spacing={1.25} sx={{ pt: 1.25, mt: 0.5, borderTop: '2px solid', borderColor: 'divider' }}>
        <Box sx={{ width: 30, height: 30, borderRadius: '50%', bgcolor: d.neto >= 0 ? '#10B98122' : '#EF444422', color: d.neto >= 0 ? '#10B981' : '#EF4444', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
          <TrendingFlatIcon sx={{ fontSize: 16 }} />
        </Box>
        <Typography sx={{ fontSize: 12.5, fontWeight: 700, flex: 1 }}>Neto</Typography>
        <Typography sx={{ fontSize: 17, fontWeight: 800, color: d.neto >= 0 ? '#10B981' : '#EF4444' }}>
