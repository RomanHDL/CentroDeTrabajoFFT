...
export default function DashboardExecKpiCard({ icon, accent, title, value, unit, footerLabel, footerValue, progressPct }) {
  return (
    <Paper
      elevation={0}
      sx={{
        height: '100%', minHeight: 128, borderRadius: '16px', p: 2,
        border: '1px solid', borderColor: alpha(accent, 0.16),
    ...
    >
      <Stack direction="row" alignItems="center" spacing={1}>
        <Box sx={{
          ...
          bgcolor: alpha(accent, 0.14), display: 'grid', placeItems: 'center', color: accent,
          '& .MuiSvgIcon-root': { fontSize: 19 },
        }}>
          {icon}
        </Box>
        <Typography sx={{ fontSize: 13.5, fontWeight: 700, color: 'text.secondary' }}>{title}</Typography>
      </Stack>

      <Stack direction="row" alignItems="baseline" spacing={0.75}>
        <Typography sx={{ fontSize: 32, fontWeight: 800, color: accent, lineHeight: 1, letterSpacing: -0.5 }}>
          {value}
        </Typography>
        {unit && <Typography sx={{ fontSize: 12.5, color: 'text.secondary', fontWeight: 600 }}>{unit}</Typography>}
      </Stack>

      <Box sx={{ mt: 'auto' }}>
        {progressPct != null && (
          <Box sx={{ height: 6, borderRadius: 999, bgcolor: alpha(accent, 0.14), overflow: 'hidden', mb: 0.5 }}>
            <Box sx={{ width: `${progressPct}%`, height: '100%', bgcolor: accent, borderRadius: 999 }} />
          </Box>
        )}
        <Typography sx={{ fontSize: 11.5, color: 'text.secondary', fontWeight: 600 }}>
          {footerLabel}{footerValue != null ? `: ${footerValue}` : ''}
        </Typography>
      </Box>
    </Paper>
  )
